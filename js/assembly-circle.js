function Assembly(stats, scaffolds, contigs) {
    ...
    if (stats.binned_scaffold_lengths) {
        ...
        this.npct_length = $.map(npct_length, function(value, index) {
            return value
        });
        this.npct_count = $.map(npct_count, function(value, index) {
            return value
        });
        ...
        if (stats.contigs) {
            ...
            this.nctg_length = $.map(nctg_length, function(value, index) {
                return value
            });
            this.nctg_count = $.map(nctg_count, function(value, index) {
                return value
            });
...
Assembly.prototype.drawPlot = function(parent_div, longest, circle_span) {
    ...
    if (this.GCs && this.Ns) {
        ...
        if (Ns[0].hasOwnProperty('mean')) {
            ...
            GCs_min = GCs.map(function(d) {
                return d.min
            })
            GCs_max = GCs.map(function(d) {
                return d.max
            })
            GCs = GCs.map(function(d) {
                return d.mean
            })
        }
        ...
        gc_circle.on('mousemove', function() {
            ...
            if (angle <= 100) {
                slow_plot = setTimeout(function() {
                    ...
                    var seg_n = Ns.slice((angle - 1) * 10, angle * 10).reduce(function(a, b) {
                        return a + b;
                    }, 0) / 10;
                    var seg_gc = GCs.slice((angle - 1) * 10, angle * 10).reduce(function(a, b) {
                        return a + b;
                    }, 0) / 10;
